...
public class AwaitEnemyTrackSelectionForArchersAttack extends StateAdapter {
    ...
    public IStates ArchersAttackTrackSelection(int value) {
        ...
        switch (value) {
            case 1:
                if ((new Dice(6).rollDice() + getDataGame().getDRM().getLadders()) > getDataGame().getEnemy().getLaddersStrength()) {
                    getDataGame().getEnemy().getLadders().Backward();
                }
                break;

            case 2:
                if ((new Dice(6).rollDice() + getDataGame().getDRM().getBatteringRam()) > getDataGame().getEnemy().getBatteringRamStrength()) {
                    getDataGame().getEnemy().getBatteringRam().Backward();
                }
                break;
            case 3:
                if ((new Dice(6).rollDice() + getDataGame().getDRM().getSiegeTower()) > getDataGame().getEnemy().getSiegeTowerStrength()) {
